...
class CFSCustomerWiseAgeing extends StatefulWidget {
  final String from;
  final String to;

  CFSCustomerWiseAgeing({this.from, this.to});

  @override
  _CFSCustomerWiseAgeingState createState() => _CFSCustomerWiseAgeingState();
}

class _CFSCustomerWiseAgeingState extends State<CFSCustomerWiseAgeing> {
  final cfsBloc = OutstandingBloc();
  AsyncSnapshot<OutstandingCustModel> as;
  TextEditingController searchc;
  bool search = false;
  List<Outstanding> _searchResultCFS = [];
  List<Outstanding> overAllDataCFS = [];

  @override
  initState(){
    super.initState();
    searchc = TextEditingController();
    cfsBloc.fetchCFSCust(widget.from, widget.to);
  }
      ...
      child: Column(
        children: [
          Container(
            ...
            child: Text(
              widget.to == "0" ? 'CFS: Ageing Outstanding\n${widget.from}' : 'CFS: Ageing Outstanding\n${widget.from}-${widget.to} days',
          ...
          StreamBuilder<OutstandingCustModel>(
            ...
            builder: (c, s){
              ...
              overAllDataCFS = as.data.outstandings;
              return Container(
                ...
                child: as.data.responseMessege.messege == null ? Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      search == false ? idcList(overAllDataCFS) : _searchResultCFS.length == 0 ? Container() : idcList(_searchResultCFS),
                      SizedBox(height: 20,)
                    ],),
                ) : Container(),
              );
  ...
  onSearchTextChanged(String text) async {
    ...
    print("search result length ${_searchResultCFS.length}");
    setState(() {});
  }
}
